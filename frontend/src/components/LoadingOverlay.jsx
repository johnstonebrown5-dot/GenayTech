...
export default function LoadingOverlay({ message = 'Loading…', transparent = false, percent = null, cardTransparent = false }) {
  ...
  const overlayBg = transparent ? '' : (isMobile ? 'bg-black/10 backdrop-blur-sm' : 'bg-white/60 backdrop-blur-sm')
  const textSize = isMobile ? 'text-xs' : 'text-sm'
  const cardStyle = cardTransparent ? 'bg-transparent border-0 shadow-none' : 'shadow-lg bg-white border border-gray-200'

  return (
    <div className={`pointer-events-none fixed inset-0 z-50 flex items-center justify-center ${overlayBg}`}> 
      <div className={`flex flex-col items-center gap-2 ${cardPad} rounded-xl ${cardStyle}`}>

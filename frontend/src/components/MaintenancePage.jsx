...
export default function MaintenancePage({ message, endsAt = null, helpPath = '/help' }){
  ...
  const logout = () => {
    try { window.localStorage.removeItem('access') } catch {}
    try { window.localStorage.removeItem('refresh') } catch {}
    try { window.localStorage.removeItem('user_data') } catch {}
    try { window.localStorage.removeItem('auth_user_id') } catch {}
    try { window.localStorage.setItem('auth_event', `logout:${Date.now()}`) } catch {}
      ...
      <div className="max-w-md w-full text-center">
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          <button
            type="button"
            onClick={logout}
            className="inline-flex items-center px-4 py-2 rounded-lg bg-white border border-gray-200 text-gray-700 text-sm font-semibold hover:bg-gray-50"
          >
            Logout
          </button>
        </div>
